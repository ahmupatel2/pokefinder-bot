...
async def get_direct_url(trackalacker_url: str, retailer: str, alert_price: float):
...
async def fetch_price_from_json(trackalacker_url: str, retailer: str):
    slug = extract_slug(trackalacker_url)
    if not slug:
        return None
...
async def on_message(message):
    ...
    if restock.get("trackalacker_url") and restock.get("retailer") and restock.get("price") is not None:
        ...
        if direct_url:
            ...
            print("[PokeFinder] No direct URL found, keeping TrackaLacker link")
